...
\textbf{Burnside's Lemma:} For a thorough overview of this result see \cite{analysis and applications of burnsides lemma jenny jin}, an account and analysis that the author found very useful. For a finite group $G=G_4$ acting on the set $S$ of shidoku boards we have $S/G_4$ denoting the set of orbits of $S$. 
...
\subsection{How many?}
We start as usual by standarising B1. This shrinks the search space by a factor of 9!. 
...
We will explore a few reductions. We have already exploited relabeling as best we can so we now focus on structural equivalences. Let us look at swapping columns and pillars. For each 
